...
def draw_init(figure: plt.Figure, location=121) -> None:
    ...
    colors = ["g" if tag else "r" for tag in y]
    plot.scatter(X[:, 0], X[:, 1], c=colors)
    figure.savefig(f"circle/0.png")
...
def draw_kms(figure: plt.Figure, location=122) -> None:
    ...
    kms = KMeans(n_clusters=2, max_iter=1000)
    # 预测样本类型
    y_sample = kms.fit_predict(X, y)
    # 获取聚类中心
    centerPoints = kms.cluster_centers_

    colors = ["g" if tag else "r" for tag in y_sample]
    plot.scatter(X[:, 0], X[:, 1], c=colors)
...
def my_draw_kms(figure: plt.Figure, location=122) -> None:
    ...
    def draw_step(tag):
        ...
        figure.savefig(f"circle/{location}.png")
    draw_step(1)
    for tag in range(10):
        for i in range(K):
    ...
    plt.legend()
    figure.savefig(f"circle/sum.png")
